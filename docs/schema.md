...
External references (knowledge tools, Jira, Slack, code permalinks) keep their normal `https://` URLs unchanged. `id` references in `supersedes` / `contradicts` / `superseded_by` are bare ids, not paths.
...
sources:                              # external pointers (knowledge tools / Jira / Slack / code permalinks). Optional.
